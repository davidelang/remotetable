...
**Preferred agent/dev test path (no device):** merge via `scripts/remotetable merge --config …`; offline files: `--backend json-book --path …` / `csv-dir`. `python3 conformance/harness.py` or `scripts/remotetable conformance`; mock push via `scripts/remotetable push --config …`. See CONTRACT Testing. CLI agents and app integrators share that one document. Conformance harness: `conformance/`.
